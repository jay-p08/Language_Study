...
void GameEngine::UpdateSprites()
{
  ...
  vector<Sprite*>::iterator siSprite;
  for (siSprite = m_vSprites.begin(); siSprite != m_vSprites.end(); /* no increment here */)
  {
    ...
    {
      ++siSprite; // only increment if not erased
    }
    if (siSprite != m_vSprites.end() && CheckSpriteCollision(*siSprite))
      (*siSprite)->SetPosition(rcOldSpritePos);
  }
}
//void GameEngine::UpdateSprites()
//{
//    // 추가 및 제거할 스프라이트를 저장할 임시 벡터
//    vector<Sprite*> vSpritesToAdd;
//    vector<Sprite*> vSpritesToKill;
//
//    // 1. 모든 스프라이트의 상태를 업데이트하고, 추가/제거 대상을 수집합니다.
//    for (Sprite* pSprite : m_vSprites)
//    {
//        // Save the old sprite position
//        RECT rcOldSpritePos = pSprite->GetPosition();
//
//        // Update the sprite
//        SPRITEACTION saSpriteAction = pSprite->Update();
//
//        // Handle the SA_ADDSPRITE sprite action
//        if (saSpriteAction & SA_ADDSPRITE)
//        {
//            // 추가할 스프라이트를 임시 벡터에 저장
//            vSpritesToAdd.push_back(pSprite->AddSprite());
//        }
//
//        // Handle the SA_KILL sprite action
//        if (saSpriteAction & SA_KILL)
//        {
//            // 제거할 스프라이트를 임시 벡터에 저장
//            vSpritesToKill.push_back(pSprite);
//        }
//    }
//
//    // 2. 제거할 스프라이트들을 처리합니다.
//    for (Sprite* pSprite : vSpritesToKill)
//    {
//        // Notify the game that the sprite is dying
//        SpriteDying(pSprite);
//
//        // m_vSprites 벡터에서 스프라이트를 찾아서 제거합니다.
//        // std::remove와 erase를 함께 사용하는 것이 효율적입니다.
//        m_vSprites.erase(std::remove(m_vSprites.begin(), m_vSprites.end(), pSprite), m_vSprites.end());
//
//        // Kill the sprite
//        delete pSprite;
//    }
//
//    // 3. 추가할 스프라이트들을 처리합니다.
//    if (!vSpritesToAdd.empty())
//    {
//        m_vSprites.insert(m_vSprites.end(), vSpritesToAdd.begin(), vSpritesToAdd.end());
//    }
//}
//---------------------------------------------------------------------------
...
void GameEngine::CleanupSprites()
{
    ...
    while (siSprite != m_vSprites.end()) {
        delete* siSprite;
        siSprite = m_vSprites.erase(siSprite); // erase returns the next valid iterator
    }
}

//void GameEngine::CleanupSprites()
//{
//    // 1. 벡터에 있는 모든 Sprite 포인터를 순회하며 객체를 삭제합니다.
//    for (Sprite* pSprite : m_vSprites)
//    {
//        delete pSprite;
//    }
//
//    // 2. 모든 포인터가 담겨있는 벡터를 비웁니다.
//    m_vSprites.clear();
//}
//---------------------------------------------------------------------------
/*
