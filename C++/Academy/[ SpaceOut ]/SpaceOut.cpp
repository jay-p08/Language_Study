...
BOOL GameInitialize(HINSTANCE hInstance)
{
  ...
  _pGame = new GameEngine(hInstance, const_cast<wchar_t*>(TEXT("Space Out 4")),
      const_cast<wchar_t*>(TEXT("Space Out 4")), IDI_SPACEOUT, IDI_SPACEOUT_SM, 600, 450);
  if (_pGame == NULL)
    return FALSE;
...
void GameStart(HWND hWindow)
{
  ...
  _pSplashBitmap = new Bitmap(hDC, IDB_SPLASH, _hInstance2);
  _pDesertBitmap = new Bitmap(hDC, IDB_DESERT, _hInstance2);
  _pCarBitmap = new Bitmap(hDC, IDB_CAR, _hInstance2);
  _pSmCarBitmap = new Bitmap(hDC, IDB_SMCAR, _hInstance2);
  _pMissileBitmap = new Bitmap(hDC, IDB_MISSILE, _hInstance2);
  _pBlobboBitmap = new Bitmap(hDC, IDB_BLOBBO, _hInstance2);
  _pBMissileBitmap = new Bitmap(hDC, IDB_BMISSILE, _hInstance2);
  _pJellyBitmap = new Bitmap(hDC, IDB_JELLY, _hInstance2);
  _pJMissileBitmap = new Bitmap(hDC, IDB_JMISSILE, _hInstance2);
  _pTimmyBitmap = new Bitmap(hDC, IDB_TIMMY, _hInstance2);
  _pTMissileBitmap = new Bitmap(hDC, IDB_TMISSILE, _hInstance2);
  _pSmExplosionBitmap = new Bitmap(hDC, IDB_SMEXPLOSION, _hInstance2);
  _pLgExplosionBitmap = new Bitmap(hDC, IDB_LGEXPLOSION, _hInstance2);
  _pGameOverBitmap = new Bitmap(hDC, IDB_GAMEOVER, _hInstance2);
...
void GameActivate(HWND hWindow)
{
  if (!_bDemo)
    ...
    _pGame->PlayMIDISong(const_cast<wchar_t*>(TEXT("")), FALSE);
}
//---------------------------------------------------------------------------
...
void GamePaint(HDC hDC)
{
  ...
  if (_bDemo)
  {
    ...
    for (int i = 0; i < 5; i++)
    {
      wsprintf(szText, L"%d", _iHiScores[i]);
...
void HandleKeys()
{
  if (!_bGameOver && !_bDemo)
  {
    ...
    if ((++_iFireInputDelay > 6) && GetAsyncKeyState(VK_SPACE) < 0)
    {
      ...
      PlaySound((LPCTSTR)IDW_MISSILE, _hInstance2, SND_ASYNC |
...
BOOL SpriteCollision(Sprite* pSpriteHitter, Sprite* pSpriteHittee)
{
    ...
    pHitter == _pJellyBitmap || pHitter == _pTimmyBitmap)))
  {
    ...
    PlaySound((LPCTSTR)IDW_LGEXPLODE, _hInstance2, SND_ASYNC |
      SND_RESOURCE);
    ...
    pHitter == _pJMissileBitmap || pHitter == _pTMissileBitmap)))
  {
    ...
    if (--_iNumLives == 0)
    {
      ...
      PlaySound((LPCTSTR)IDW_GAMEOVER, _hInstance2, SND_ASYNC |
        SND_RESOURCE);
      _bGameOver = TRUE;
      _iGameOverDelay = 150;
...
void SpriteDying(Sprite* pSprite)
{
    ...
    pSprite->GetBitmap() == _pTMissileBitmap)
  {
    // Play the small explosion sound
    if (!_bDemo)
      PlaySound((LPCTSTR)IDW_SMEXPLODE, _hInstance2, SND_ASYNC |
        SND_RESOURCE | SND_NOSTOP);
...
void NewGame()
{
  ...
  else
  {
    ...
    _pGame->PlayMIDISong(const_cast<wchar_t*>(TEXT("Music.mid")));
  }
}
//---------------------------------------------------------------------------
void AddAlien()
...
BOOL WriteHiScores()
{
  ...
  for (int i = 0; i < 5; i++)
  {
    ...
    WCHAR cData[6];
    wsprintf(cData, L"%05d", _iHiScores[i]);
